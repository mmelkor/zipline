...
class AssetFinder(object):
    ...
    def _resolve_futures(self, contracts, as_of_date, contract_num):
        """
        Takes the list of all contracts for a given root_symbol and
        returns the contract/contracts we want.
        """

        # TODO: reasonable to do this every time?
        valid_contracts = [
            c for c in contracts if c.expiration_date > as_of_date
        ]
    ...
    def lookup_future(self, root_symbol, as_of_date, contract_num=None):
        """ Find a specific futures contract or a list of valid contracts for
        a type of future.
        ...
        contract_num : int or None
            1 for the primary contract, 2 for the secondary, etc., relative to
            as_of_date.
            None for a list of all valid futures relative to as_of_date.
        """

        if contract_num is not None and contract_num < 1:
            return None
        ...
        try:
            ...
            return self._resolve_futures(contracts, as_of_date, contract_num)
        except KeyError:
            return None
